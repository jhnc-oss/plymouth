...
create_control (void)
{
        ...
        error = FT_Set_Char_Size (label->face, 12 << 6, 0, 100, 0);
        if (error)
        {
...
static FT_Int width_of_line (ply_label_plugin_control_t *label,
                            const char                 *text)
{
        FT_Int width = 0;
        FT_Int last_left = 0;
...
size_control (ply_label_plugin_control_t *label)
{
        ...
        if ((long) label->area.width < label->alignment_width)
            label->area.width = label->alignment_width;
}
              ...
              unsigned long               height)
{
        ...
        while (*cur_c)
        {
                ...
                if (label->alignment == PLY_LABEL_ALIGN_CENTER)
                    pen.x += (label->alignment_width - width_of_line (label, cur_c)) << 5;
                else if (label->alignment == PLY_LABEL_ALIGN_RIGHT)
                    pen.x += (label->alignment_width - width_of_line (label, cur_c)) << 6;
...
set_width_for_control (ply_label_plugin_control_t *label,
                       long                        width)
{
        if (label->alignment_width != width) {
                label->alignment_width = width;
                trigger_redraw (label, true);
        }
}
                      ...
                      const char                 *fontdesc)
{
        ...
        if (size_in_pixels)
                ...
                FT_Set_Char_Size (label->face, size << 6, 0, 100, 0);
